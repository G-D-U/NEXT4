...
'''
mm = '''
**- عليك الانضمـام في قنـاة السـورس اولاً**  @G_D_U_VIP
'''
...
keyboard = [
  ...
  [
    Button.url("𝗭𝗧𝗵𝗼𝗻™ 𓅛", "https://t.me/G_D_U_VIP")
    ]
]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if len(i) > 1:
        ...
        file.write(i + "\n\n**- بواسطـة next @G_D_U_VIP**")
        file.close()
        await bot.send_file(event.chat_id, "session.txt")
        system("rm -rf session.txt")
      else:
        await event.reply(i + "\n\n**- شكـراً لـ استخدامـك سـورس NEXT ❤️** \n/hack", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply(i + "\n\n**- شكـراً لـ استخدامـك سـورس NEXT ❤️**", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      try:
        ...
        await event.respond(f"**- ارسل هذا الخطأ الى @G_D_U \n- الخطـأ** str(e)\n")
...
async def start(event):
    keyboard = [
      ...
      [
        Button.url("قنـاة السـورس", "https://t.me/G_D_U_VIP")
        ]
    ]
